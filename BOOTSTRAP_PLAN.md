...
| 字符串 | ⚠️ 基础 | 映射为 `const char*`，不可变，提供 10 个内建方法 |
| 数组 | ⚠️ 基础 | `Array<T>` 动态数组，支持 add/get/set/size/removeAt |
| 映射 | ⚠️ 基础 | `Map<K,V>` 哈希映射，FNV-1a + 开放寻址法 |
| 泛型 | ✅ 可用 | 单态化实现，支持类型约束 `T extends Comparable` |
| 函数指针 | ✅ 可用 | `void(int, String)` 形式 |
| 枚举/代数类型 | ❌ 缺失 | 无 enum、无 tagged union、无 sum type |
| 元组 | ❌ 缺失 | 无 tuple 类型 |
...
| throw | ✅ 完整 | — |
| switch/match | ❌ 缺失 | 无模式匹配，无 switch 语句 |
| for-each 迭代 | ❌ 缺失 | 无集合遍历语法 |
| do-while | ❌ 缺失 | — |
...
| 文件 I/O | ❌ 缺失 | 无 File 类，需通过 extern 调用 C 函数 |
| 垃圾回收 | ⚠️ 框架 | gc_init/gc_alloc/gc_mark/gc_sweep，标记-清除框架 |
| 数学库 | ❌ 缺失 | 无 Math 类 |
| 字符处理 | ❌ 缺失 | 无 Character 工具类 |
...
**当前状况：** Lemon 完全没有 enum 类型、tagged union 或模式匹配。

**最低需求：**
- 密封类（sealed class）或 enum class，支持变体携带数据
- match 表达式，支持变体解构和穷尽性检查
- 作为过渡方案：可用类继承 + instanceof 模拟，但极其冗长
...
**当前状况：** String 映射为 `const char*`，不可变。虽有 10 个内建方法，但缺少：
- 字符串拼接运算符（`+` 或 `concat`）
- 可变字符串构建器（StringBuilder）
- 字符串分割（split）
...
**最低需求：**
- StringBuilder 类（可变字符串，支持 append/toString）
- 字符串 `+` 拼接运算符
- char 与 String 的互转
- 字符串分割和格式化
...
**当前状况：** 无任何文件 I/O 抽象。虽然可通过 `extern` 声明 C 函数（fopen/fread/fclose/fwrite），但这不是 Lemon 层面的解决方案，且缺乏类型安全。
...
**当前状况：** `char` 类型存在但无任何内建方法。没有 `Character.isDigit()`、`Character.isAlpha()` 等工具。

**最低需求：**
- Character 工具类：isDigit/isAlpha/isWhitespace/isAlphanumeric
- char 与 int 的互转（ASCII 码）
- 字符比较
...
### 2.6 switch/match 语句（严重性：🟠 严重）

**编译器需求：** 编译器中大量使用基于 Token 类型的分支分发。当前 Rust 实现中，词法分析器和语法分析器都大量使用 match 表达式。没有 switch/match，只能用 if-else 链，代码冗长且效率低。

**当前状况：** 完全没有 switch 或 match 语句。

**最低需求：**
- 基本的 switch 语句（支持 int/String/enum 匹配）
- 如果有 enum，则需要 match 表达式支持变体解构
...
| 特性 | 优先级 | 实现方案 | 预计工作量 |
|------|--------|----------|-----------|
| 枚举类型（enum class） | P0 | 新增 `enum` 语法，编译为 tagged union（struct + kind 字段 + union payload） | 3-4 周 |
| match 表达式 | P0 | 新增 `match` 语法，编译为 switch/if-else 链 + 变体解构 | 2-3 周 |
| switch 语句 | P0 | 新增 `switch`/`case` 语法，编译为 C switch 或 if-else 链 | 1 周 |
| 字符串拼接 `+` | P0 | 运算符重载或编译器内建处理，生成 strcat/动态拼接代码 | 1 周 |
| for-each 循环 | P1 | `for (T item in collection)` 语法，编译为迭代器调用 | 1 周 |
| 可空类型 `T?` | P1 | 语法糖，编译为带 null 检查的引用或 Option 结构体 | 1-2 周 |
...
| 类 | 优先级 | 方法 | 预计工作量 |
|----|--------|------|-----------|
| StringBuilder | P0 | new/append/appendChar/appendInt/appendLong/appendFloat/appendDouble/toString/clear/length | 1 周 |
| File | P0 | open/close/readAll/writeAll/exists/readLine/readBytes | 1-2 周 |
| Character | P0 | isDigit/isAlpha/isWhitespace/isAlphanumeric/isUpper/isLower/toUpper/toLower/isPrintable | 3 天 |
| Stack | P1 | new/push/pop/peek/isEmpty/size | 3 天 |
| Set | P1 | new/add/contains/remove/size | 3 天 |
...
- [ ] enum 类型可声明、实例化、在 match 中解构
- [ ] match 表达式编译正确，穷尽性检查通过
- [ ] switch 语句支持 int/String/enum 匹配
- [ ] 字符串 `+` 拼接运算符工作正常
- [ ] StringBuilder 通过单元测试（构建 1000+ 字符的长字符串）
- [ ] File 类可读写文本文件
- [ ] Character 工具类所有方法正确
- [ ] 现有测试套件全部通过（无回归）
...
| 阶段 | 预计时间 | 累计时间 |
|------|---------|---------|
| Phase 1：补齐核心缺失功能 | 8-12 周 | 8-12 周 |
| Phase 2：重写词法分析器 | 2-3 周 | 10-15 周 |
| Phase 3：重写语法分析器 | 5-6 周 | 15-21 周 |
| Phase 4：重写语义分析器 | 5-6 周 | 20-27 周 |
| Phase 5：重写代码生成器 | 16-20 周 | 36-47 周 |
| Phase 6：自举验证 | 5-7 周 | 41-54 周 |
...
| 风险 | 概率 | 影响 | 缓解策略 |
|------|------|------|---------|
| enum/match 实现复杂度超预期 | 高 | 延迟 Phase 1 | 先用类继承 + instanceof 模拟 enum，后续再引入原生 enum |
| 泛型单态化导致编译时间爆炸 | 中 | 编译自身时极慢 | 限制泛型实例化深度；引入泛型共享（box 化）策略 |
| C 代码生成器重写工作量过大 | 高 | 延迟 Phase 5 | 优先只实现 C 后端；其余后端延后 |
| 运行时库性能不足 | 中 | 编译速度慢 | 用 C 实现关键运行时函数（StringBuilder、File I/O），Lemon 声明 extern 调用 |
...
```
M1: enum + match + switch + StringBuilder + File + Character 可用
    └── Phase 1 完成，Lemon 具备编写编译器组件的基础能力

M2: Lemon 版 Lexer 通过全部测试
    └── Phase 2 完成，首次用 Lemon 编译器组件替代 Rust 组件
...
| 优先级 | 特性 | 用途 |
|--------|------|------|
| P0 | enum 类型（携带数据的变体） | 表达 AST 节点、Token 类型、错误类型 |
| P0 | match 表达式（变体解构） | Token/AST 分发 |
| P0 | switch 语句 | 简单值分发 |
| P0 | StringBuilder 类 | 代码生成输出 |
| P0 | File I/O 类 | 读写源文件和输出文件 |
| P0 | Character 工具类 | 词法分析字符判断 |
| P0 | 字符串 + 拼接 | 消息拼接、代码生成 |
| P0 | String.charAt() | 逐字符访问源码 |
| P1 | Option<T> / T? | 可选值安全表达 |
| P1 | for-each 循环 | 集合遍历 |
| P1 | Stack<T> 类 | 作用域栈、解析栈 |
| P1 | Set<T> 类 | 接口名集合、去重 |
| P1 | String.startsWith/endsWith | 前缀匹配 |
| P2 | 字符串插值 | 代码生成模板 |
| P2 | 运算符重载 | 自定义比较 |
| P2 | 元组类型 | 多返回值 |
| P2 | 内联函数 | 性能优化 |
